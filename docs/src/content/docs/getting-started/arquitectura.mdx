...
- **`openubl.signer`** — Firma digital XAdES-EPES con certificado X.509 y clave privada en formato PEM (`sign_ubl_xml`).
- **`openubl.validator`** — `SunatValidator` aplica reglas y esquemas XSD de SUNAT sobre el XML generado.
- **`openubl.packager`** — Empaqueta el XML firmado en un ZIP con la convención de nombres de SUNAT (`package_invoice`, `package_voided_documents`, etc.).

Este motor funciona como biblioteca nativa: importas, creas el modelo, enriqueces, renderizas, firmas, validas y empaquetas sin necesidad de red.
...
flowchart TB
    ...
    C -->|signer| D[XML firmado<br/>XAdES-EPES]
    D -->|validator| E{¿Válido?}
    E -->|Sí| F[ZIP SUNAT<br/>packager]
    E -->|No| G[Errores de validación]
```
