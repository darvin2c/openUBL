...
```text
¿Tu proyecto es TypeScript?
  Sí → Usa el SDK TypeScript (@openubl/sdk).
  No → ¿Tu proyecto es Python?
         Sí → ¿Varios sistemas necesitan generar documentos?
                Sí → API REST.
                No → Python nativo.
         No → API REST (la llaman desde cualquier lenguaje).
```
...
- Si elegiste Python nativo, empieza con [Tu primer documento](./primer-documento/).
- Si elegiste la API REST o el SDK TypeScript, primero [instala](./instalacion/) y levanta el servidor.
- Si tienes dudas durante la integración, consulta la [guía de troubleshooting](../guides/troubleshooting/).
