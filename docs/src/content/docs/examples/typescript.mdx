...
```typescript
import { createInvoice, signXml } from "@openubl/sdk";
...
async function createAndSign() {
  // 1. Crear el XML
  const { data: createData, error: createError } = await createInvoice({
    body: invoice,
  });
  ...
  const { data: signData, error: signError } = await signXml({ body });
